...
public class TwoWayNestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild {
    ...
    public void flingVertical(int velocityY) {
        if (getChildCount() > 0) {
            ...
            mScroller.fling(scrollX, getScrollY(), 0, velocityY, scrollX, scrollX, 0, Math.max(0, bottom - height),
                    0, height / 2);
    ...
    public void flingHorizontal(int velocityX) {
        if (getChildCount() > 0) {
            ...
            mScroller.fling(getScrollX(), scrollY, velocityX, 0, 0, Math.max(0, right - width), scrollY, scrollY,
                    width / 2, 0);
            ViewCompat.postInvalidateOnAnimation(this);
        }
    }
    ...
    static class SavedState extends BaseSavedState {
        ...
        public String toString() {
            return "HorizontalScrollView.SavedState{" + Integer.toHexString(System.identityHashCode(this))
    ...
    static class AccessibilityDelegate extends AccessibilityDelegateCompat {
            ...
            switch (action) {
                case AccessibilityNodeInfoCompat.ACTION_SCROLL_FORWARD: {
                    ...
                    final int targetScrollX = Math.min(nsvHost.getScrollX() + viewportWidth,
                            nsvHost.getHorizontalScrollRange());
                ...
                case AccessibilityNodeInfoCompat.ACTION_SCROLL_BACKWARD: {
                    final int viewportWidth = nsvHost.getWidth() - nsvHost.getPaddingLeft() - nsvHost.getPaddingRight();
                    final int targetScrollX = Math.max(nsvHost.getScrollY() - viewportWidth, 0);
                    final int viewportHeight = nsvHost.getHeight() - nsvHost.getPaddingBottom()
                            - nsvHost.getPaddingTop();
        ...
        public void onInitializeAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            final boolean scrollable = nsvHost.getVerticalScrollRange() > 0
                    || nsvHost.getHorizontalScrollRange() > 0;
