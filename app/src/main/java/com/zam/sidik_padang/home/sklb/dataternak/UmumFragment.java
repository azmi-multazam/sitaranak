...
import com.zam.sidik_padang.home.sklb.dataternak.vm.DataTernakUmum;
import com.zam.sidik_padang.home.sklb.dataternak.vm.DataTernakViewModel;

public class UmumFragment extends SklbBaseFragment implements DataTernakAdapter.OnTernakItemClickListener {
        ...
        viewModel.getResponseData().observe(getViewLifecycleOwner(), responseData -> {
            if (responseData != null) {
                if (responseData.success && responseData.data != null) {
                    String datas = listToString(responseData.data.getDataTernakUmum());
    ...
    public List<DataTernakPetugas> stringToList(String data) {
        if (data == null) {
            return Collections.emptyList();
        }

        Type listType = new TypeToken<List<DataTernakPetugas>>() {
        }.getType();

        return new Gson().fromJson(data, listType);
    }

    public String listToString(List<DataTernakUmum> someObjects) {
