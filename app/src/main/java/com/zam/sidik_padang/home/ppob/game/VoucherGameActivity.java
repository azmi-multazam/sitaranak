package com.zam.sidik_padang.home.ppob.game;
...
import com.zam.sidik_padang.home.ppob.isipulsa.ProdukPilihan;
...
import com.zam.sidik_padang.util.data.VoucherGame;

/**
 * Created by supriyadi on 7/9/17.
 */

public class VoucherGameActivity extends BaseLogedinActivity {
    ...
    private int selectedJenisVoucher = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listJenisVoucher.add("- pilih jenis voucher -");
        listJenisVoucher.addAll(DataHelper.getDataHelper(this).getJenisVoucherGame());
        adapterSpinnerJenisVoucher = new SpinnerAdapter(getResources(), listJenisVoucher);
        AppCompatSpinner spinner = findViewById(R.id.activity_isi_pulsa_SpinnerJenisVoucher);
        ...
        apiHelper.getJenisVoucherGame(new ApiHelper.OnDoneListener() {
            ...
            public void onDone(boolean success) {
                if (success) {
                    ...
                    listJenisVoucher.addAll(DataHelper.getDataHelper(VoucherGameActivity.this).getJenisVoucherGame());
                    adapterSpinnerJenisVoucher.notifyDataSetChanged();
                }
            }
        });

        spinnerProdukPilihan = findViewById(R.id.activity_isi_pulsa_SpinnerProduk);
        listProdukPilihan = new ArrayList<>();
        listProdukPilihan.add("--pilih nominal--");
        ...
        edittextNomorHp = findViewById(R.id.activity_isi_pulsa_EditTextNomorHp);
        editTextPin = findViewById(R.id.activity_isi_pulsa_EditTextPin);
        spinnerNomorTransaksi = findViewById(R.id.activity_isi_pulsa_SpinnerNomorTransaksi);
    ...
    private void loadProdukPilihan() {
        if (selectedJenisVoucher == 0 || !Util.isInternetAvailible(this)) return;
        String url = Config.URL_PPOB + "userid=" + user.userid + "&aksi=5&code=" + ((VoucherGame) listJenisVoucher.get(selectedJenisVoucher)).code;
        debug(getClass(), "load pilihan nominal game url= " + url);
        ...
        VolleyStringRequest request = new VolleyStringRequest(url, new VolleyStringRequest.Callback() {
            ...
            public void onResponse(JsonObject jsonObject) {
                ...
                debug(getClass(), "Load pilihan nominal game response= " + jsonObject);
    ...
    private void prosesPembelian() {
        ...
        String url = Config.URL_PROSES +
                "aksi=5&userid=" + user.userid +
                "&token=" + editTextPin.getText().toString().trim() +
                "&kode=" + ((VoucherGame) listJenisVoucher.get(selectedJenisVoucher)).code +
		...
		*/
        editTextPin.setText("");
        debug(getClass(), "Proses beli Voucher game url= " + url);
        ...
        VolleyStringRequest request = new VolleyStringRequest(url, new VolleyStringRequest.Callback() {
            ...
            public void onResponse(final JsonObject jsonObject) {
                ...
                if (isResummed)
                    new androidx.appcompat.app.AlertDialog.Builder(VoucherGameActivity.this)
    ...
    private class MySpinnerListener extends SpinnerListener {
        ...
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
            if (viewId == R.id.activity_isi_pulsa_SpinnerJenisVoucher) {
                selectedJenisVoucher = position;
                if (selectedJenisVoucher == 0) {
